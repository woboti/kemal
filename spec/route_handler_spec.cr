...
describe "Kemal::RouteHandler" do
  ...
  context "LRU cache" do
    it "evicts least recently used entries instead of clearing entirely" do
      ...
      Kemal.config.route_handler.cached_routes = Kemal::LRUCache(String, Radix::Result(Kemal::Route)).new(small_capacity)
      ...
      0.upto(small_capacity - 1) do |i|
        Kemal.config.route_handler.lookup_route("GET", "/lru_eviction_#{i}")
      end

      Kemal.config.route_handler.cached_routes.size.should eq small_capacity
    ...
    it "retains recently used keys and evicts the least recently used" do
      ...
      # Fill cache with 0..3
      0.upto(3) do |i|
        Kemal.config.route_handler.lookup_route("GET", "/lru_recency_#{i}")
      end
      ...
      Kemal.config.route_handler.lookup_route("GET", "/lru_recency_4")

      # Insert 5 -> should evict the other of {2,3}
      Kemal.config.route_handler.lookup_route("GET", "/lru_recency_5")
      ...
      Kemal.config.route_handler.lookup_route("GET", "/lru_recency_0").found?.should be_true
      Kemal.config.route_handler.lookup_route("GET", "/lru_recency_1").found?.should be_true
    end

    it "caches HEAD fallback GET lookups without growing beyond 1 for same path" do
      cap = 16
      Kemal.config.route_handler.cached_routes = Kemal::LRUCache(String, Radix::Result(Kemal::Route)).new(cap)
      ...
      Kemal.config.route_handler.lookup_route("HEAD", "/head_fallback").found?.should be_true
      Kemal.config.route_handler.cached_routes.size.should eq 1
    ...
    it "keeps size capped under heavy churn with large capacity" do
      large_capacity = 4096
      Kemal.config.route_handler.cached_routes = Kemal::LRUCache(String, Radix::Result(Kemal::Route)).new(large_capacity)
      ...
      0.upto(11999) do |i|
        Kemal.config.route_handler.lookup_route("GET", "/lru_heavy_#{i}")
      end

      Kemal.config.route_handler.cached_routes.size.should eq large_capacity
      ...
      12000.upto(14000) do |i|
        ...
        Kemal.config.route_handler.lookup_route("GET", "/lru_heavy_more_#{i}")
      end
